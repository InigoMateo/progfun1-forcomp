...
class AnagramsSuite extends munit.FunSuite:
  ...
  test("wordAnagrams eat"){
    assertEquals(wordAnagrams("eat"), List("ate", "eat", "tea"))
  }
  ...
  test("sentence anagrams: Lab (10pts)") {
    val sentence = List("Lab")
    val anas = List(List("lab"))
    assertEquals(sentenceAnagrams(sentence), anas)
  }
