...
object Anagrams extends AnagramsInterface:
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x match
    ...
    case xs :: xs1 =>
      ...
      val resultMap = yMap.foldLeft(xMap)((xMap, yElem) =>
        if (xMap.contains(yElem._1) && xMap.apply(yElem._1) >= yElem._2) xMap.updated(yElem._1, xMap(yElem._1) - yElem._2)
        else xMap)
      resultMap.filter((char, int) => int > 0).toList
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def anagrams (occurrences: Occurrences): List[Sentence] =
      if (sentence.isEmpty) List(List())
          ...
          rest <- anagrams(subtract(sentenceOccurrences(sentence), combination))
